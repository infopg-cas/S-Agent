...
class Agent:
    def __init__(self, api_key: str = os.environ.get('OPENAI_API_KEY'), model: str = 'gpt-4') -> None:
        ...
        self.brain = Brain(
            api_key=api_key,
            model=model,
            anonymize=False)  # I know it violates the dependency injection rule, but it is not a big issue here.
    ...
    def observation(self) -> None:
        ...
        check_final = self.brain.generate(
            prompt=f"Is {self.background_info()} enough to finally answer to this request: {self.messages[0]}",
